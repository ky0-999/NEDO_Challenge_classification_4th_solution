...
# ウェーブレット変換
def wavelet_transform(signal, wavelet='sym4', level=1):
    """
    信号に対してWavelet変換を行う関数
    signal: 時系列データ (numpy array) - 形状 (channels, time_length)
    wavelet: Waveletの種類 (str)
    level: 変換のレベル (int)
    """
    coeffs = [pywt.wavedec(channel, wavelet, level=level) for channel in signal]
    # 各チャンネルごとのWavelet変換後の係数を結合してnp.arrayに戻す
    transformed_signal = np.array([np.concatenate(c) for c in coeffs])
    return transformed_signal.astype(np.float32)
